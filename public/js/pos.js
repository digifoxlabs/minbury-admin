...
$(document).ready(function() {
    ...
    $('button.pos-express-finalize').click(function() {
        ...
        if (pay_method == 'card') {
            ...
            $('div#upi_details_modal').modal('show');
        }
    ...
    //On Save UPI Details
    $('button#pos-save-upi').click(function() {
        $('input#payment[0][transaction_no_1]').val($('#transaction_no_1_0').val());


        $('div#upi_details_modal').modal('hide');
        pos_form_obj.submit();
    });
    ...
    $(document).on('change', '#contact_shipping_address_id', function() {
        var selected = $(this).find(':selected');
        var shippingAddress = selected.data('shipping_address');

        if (typeof shippingAddress !== 'undefined') {
            $('#shipping_address_modal').val(shippingAddress);
...
function set_default_customer() {
    ...
    var default_customer_shipping_addresses = $('#default_customer_shipping_addresses').val();
    ...
    set_contact_shipping_addresses(
        default_customer_shipping_addresses ? JSON.parse(default_customer_shipping_addresses) : [],
...
function update_shipping_address(data) {
    var defaultShippingAddress = data.shipping_address ? data.shipping_address : '';
    var selectedShippingAddress = defaultShippingAddress;
    var currentCustomerId = $('#customer_id').val();
    var defaultCustomerId = $('#default_customer_id').val();
    var currentShippingAddress = $('#shipping_address').val();

    if (
        $('#edit_sell_form').length &&
        currentShippingAddress &&
        defaultCustomerId &&
        currentCustomerId &&
        String(currentCustomerId) === String(defaultCustomerId)
    ) {
        selectedShippingAddress = currentShippingAddress;
    }
    ...
    if ($('#shipping_address_div').length) {
        ...
        shipping_address += ',<br>' + selectedShippingAddress;
    ...
    set_contact_shipping_addresses(data.shipping_addresses || [], selectedShippingAddress);
    $('#shipping_address_modal').val(selectedShippingAddress);
    $('#shipping_address').val(selectedShippingAddress);
}

function set_contact_shipping_addresses(shippingAddresses, selectedAddress) {
    var $select = $('#contact_shipping_address_id');
    if (!$select.length) {
        return;
    }

    $select.empty();
    $select.append($('<option>', { value: '', text: LANG.please_select }));
    ...
    $.each(shippingAddresses, function(index, address) {
        var label = address.label ? address.label : 'Address ' + (index + 1);
        var option = $('<option>', {
            value: index,
            text: label
        }).attr('data-shipping_address', address.shipping_address);

        if (selectedAddress && address.shipping_address === selectedAddress) {
            selectedValue = String(index);
        } else if (!selectedValue && address.is_default) {
            selectedValue = String(index);
        }
