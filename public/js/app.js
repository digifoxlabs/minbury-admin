$(document).ready(function() {
    ...
    $('.contact_modal').on('shown.bs.modal', function(e) {
        ...
        $(document).on('click', '.add_shipping_address_row', function() {
            var container = $(this).closest('.shipping_addr_div').find('.contact_shipping_addresses').first();
            var nextIndex = parseInt(container.data('next-index'), 10) || 0;
            var shippingAddressLabel = typeof LANG.shipping_address !== 'undefined' ? LANG.shipping_address : 'Shipping address';
            var rowHtml =
                '<div class="row contact_shipping_address_row mb-10">' +
                    '<div class="col-md-3">' +
                        '<div class="form-group">' +
                            '<label for="shipping_addresses_' + nextIndex + '_label">Label:</label>' +
                            '<input class="form-control" placeholder="Home / Office / Warehouse" name="shipping_addresses[' + nextIndex + '][label]" type="text">' +
                            '<input class="shipping_address_position" name="shipping_addresses[' + nextIndex + '][position]" type="hidden">' +
                        '</div>' +
                    '</div>' +
                    '<div class="col-md-7">' +
                        '<div class="form-group">' +
                            '<label for="shipping_addresses_' + nextIndex + '_address">' + shippingAddressLabel + ':</label>' +
                            '<textarea class="form-control shipping_address_value" rows="2" placeholder="' + shippingAddressLabel + '" name="shipping_addresses[' + nextIndex + '][shipping_address]"></textarea>' +
                        '</div>' +
                    '</div>' +
                    '<div class="col-md-2">' +
                        '<div class="form-group" style="margin-top: 25px;">' +
                            '<div class="radio">' +
                                '<label><input type="radio" name="shipping_addresses_default" value="' + nextIndex + '"> Default</label>' +
                            '</div>' +
            ...
            if (container.find('input[name="shipping_addresses_default"]:checked').length === 0) {
                container.find('input[name="shipping_addresses_default"][value="' + nextIndex + '"]').prop('checked', true);
            }
        });

        $(document).on('click', '.remove_shipping_address_row', function() {
            var container = $(this).closest('.contact_shipping_addresses');
            var row = $(this).closest('.contact_shipping_address_row');
            var removedWasDefault = row.find('input[name="shipping_addresses_default"]').is(':checked');

            row.remove();

            if (container.find('.contact_shipping_address_row').length === 0) {
                container.closest('.shipping_addr_div').find('.add_shipping_address_row').trigger('click');
                return;
            }

            if (removedWasDefault) {
                container.find('input[name="shipping_addresses_default"]').first().prop('checked', true);
            }
        });
