...
use App\Contact;
use App\ContactShippingAddress;
use App\Transaction;
use DB;
...
class ContactUtil extends Util
{
    ...
    public function createNewContact($input)
    {
        ...
        unset($input['shipping_addresses']);
        ...
        if ($count == 0) {
            ...
            $this->syncShippingAddresses($contact, $shipping_addresses);
    ...
    protected function syncShippingAddresses(Contact $contact, array $shipping_addresses = [])
    {
        $contact->shippingAddresses()->delete();
        ...
        foreach ($shipping_addresses as $index => $shipping_address) {
            if (empty($shipping_address['shipping_address'])) {
                continue;
            }

            $rows[] = new ContactShippingAddress([
                'label' => $shipping_address['label'] ?? null,
                'shipping_address' => $shipping_address['shipping_address'],
                'position' => $shipping_address['position'] ?? null,
                'is_default' => ! empty($shipping_address['is_default']),
                'sort_order' => $index,
            ]);
        }

        if (! empty($rows)) {
            $contact->shippingAddresses()->saveMany($rows);
        }
    }
